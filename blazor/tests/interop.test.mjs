...
test('all exports, nested constructors and instance this binding remain accessible', async () => {
  ...
  assert.equal(await s.call(m, 'add', [3]), 7);
  await s.set(m, 'value', 10);
  assert.equal(s.get(m, 'value'), 10);
  assert.equal(await s.invoke('sum', [1, 2]), 3);
  await s.dispose(); assert.equal(m.disposals, 1);
  assert.throws(() => s.get(m, 'value'), /disposed/);
});
test('reference identity survives recursively resolved arguments', async () => {
  const s = new Session({ api: { identity: v => v } });
  const item = { id: 3, nested: { n: 4 } };
...
test('module, property, setter, constant and async .NET callbacks require no eval', async () => {
  const s = new Session({ api: { apply: (fn, ...args) => fn(...args) } });
  const item = { id: 1 };
...
test('event ordering, unsubscribing and session ownership are deterministic', async () => {
  ...
  const sub = await s.subscribe(m, 'Changed', receiver(values));
  m.Changed.emit({ n: 1 }); m.Changed.emit({ n: 2 }); await tick();
  assert.deepEqual(values, [{ n: 1 }, { n: 2 }]);
  sub.dispose(); sub.dispose(); m.Changed.emit({ n: 3 }); await tick(); assert.equal(values.length, 2);
...
test('DOM event subscriptions forward detail and detach', async () => {
  ...
  const sub = await s.subscribe(target, 'dom:change', receiver(values));
  target.dispatchEvent(new CustomEvent('change', { detail: { text: 'hello' } })); await tick(); assert.deepEqual(values, [{ text: 'hello' }]);
...
test('late asynchronous mount is cleaned up when its owner is disposed', async () => {
  ...
  const pending = s.mount({}, {}); await tick(); await s.dispose(); complete(model);
  await assert.rejects(pending, /disposed/); assert.equal(model.disposals, 1);
});
test('snapshot handles cycles, maps, typed arrays and errors without losing sibling values', () => {
  ...
  assert.deepEqual(snapshot(new Map([['a', 2]])), [['a', 2]]);
  assert.deepEqual(snapshot(new Uint8Array([1, 2])), [1, 2]);
...
test('unsafe prototype paths and non-callable exports fail explicitly', async () => {
  const s = new Session({ api: { value: 1 } });
  await assert.rejects(s.set({}, '__proto__.polluted', true), /Invalid member/);
  assert.throws(() => s.get({}, 'constructor'), /Invalid member/);
  await assert.rejects(s.invoke('value'), /Not callable/);
  await assert.rejects(s.construct('missing'), /Not a constructor/);
